...
A evolução prompt 09 adiciona provisionamento local explícito de duas identidades de teste, com concessão Basic marcada `development-demo-access` e evento de auditoria. O comando é recusado sem `--environment Development`, não promove conta comum preexistente e não registra senhas. A evidência permanece **não executada neste ambiente** por ausência de SDK .NET e PostgreSQL autorizado; persistência, login HTTP e MFA não são inferidos da implementação.
