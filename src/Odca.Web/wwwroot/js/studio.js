...
if (studio) {
  ...
  const loadComments=async()=>{const items=await request(`${studio.dataset.commentsUrl}?resolved=${studio.querySelector("[data-resolved]").checked}`);const target=studio.querySelector("[data-comments]");target.replaceChildren(...items.map(item=>{const article=document.createElement("article");article.className=`comment${item.parentId?" reply":""}`;const heading=document.createElement("strong");heading.textContent=`${item.author} · ${item.reference}${item.referenceLocated?"":" · trecho não localizado"}`;const body=document.createElement("p");body.textContent=item.body;const reply=document.createElement("button");reply.type="button";reply.className="button button-small button-link";reply.textContent="Responder";reply.addEventListener("click",()=>{commentParentId=item.id;const form=studio.querySelector("[data-comment-form]");form.elements.reference.value=item.reference;form.elements.body.focus();});const button=document.createElement("button");button.type="button";button.className="button button-small button-link";button.textContent=item.resolved?"Reabrir":"Resolver";button.addEventListener("click",async()=>{await request(`${studio.dataset.commentsUrl}/${item.id}/${item.resolved?"reopen":"resolve"}`,{method:"POST"});await loadComments();});article.append(heading,body,reply,button);return article;}));};
