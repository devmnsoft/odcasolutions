...
public sealed class ConsumptionController(OdcaApiClient api):Controller
{
 ...
 public async Task<IActionResult> CreateStorageRequest(Guid tenantId,Guid packageId,int quantity,Guid idempotencyKey,CancellationToken ct){var token=await HttpContext.GetTokenAsync("access_token");if(token is null)return Challenge();var result=await api.RequestStorageAsync(token,tenantId,new(packageId,quantity,idempotencyKey),ct);if(!result.Succeeded){TempData["Error"]=result.UserMessage("Não foi possível registrar a solicitação. Revise o pacote e tente novamente.");}else TempData["Success"]="Solicitação registrada. A capacidade só mudará após aprovação explícita.";return RedirectToAction(nameof(Index),new{tenantId});}
}
