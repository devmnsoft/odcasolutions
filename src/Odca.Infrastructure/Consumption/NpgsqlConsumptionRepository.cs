...
public sealed class NpgsqlConsumptionRepository(NpgsqlDataSource dataSource) : IConsumptionRepository
{
    ...
    { await using var c=await dataSource.OpenConnectionAsync(cancellationToken);await using var tx=await c.BeginTransactionAsync(cancellationToken);await Context(c,tx,actorId,tenantId,cancellationToken);if(!platformAccess&&!await Allowed(c,tx,actorId,tenantId,"tenant.billing.read",cancellationToken))return [];var rows=(await c.QueryAsync<AdditionalStorageRequestRow>(new CommandDefinition(RequestSql+" WHERE r.tenant_id=@tenantId ORDER BY r.requested_at DESC",new{tenantId},tx,cancellationToken:cancellationToken))).AsList();await tx.CommitAsync(cancellationToken);return rows.Select(ToContract).ToArray(); }
    ...
    { await using var c=await dataSource.OpenConnectionAsync(cancellationToken);await using var tx=await c.BeginTransactionAsync(cancellationToken);await Context(c,tx,actorId,tenantId,cancellationToken);if(!await Allowed(c,tx,actorId,tenantId,"tenant.billing.manage",cancellationToken))return null;
      ...
      var row=await c.QuerySingleOrDefaultAsync<AdditionalStorageRequestRow>(new CommandDefinition(RequestSql+" WHERE r.tenant_id=@tenantId AND r.idempotency_key=@key AND r.package_version_id=@packageId AND r.quantity=@quantity",new{tenantId,key=request.IdempotencyKey,packageId=request.PackageId,quantity=request.Quantity},tx,cancellationToken:cancellationToken));if(row is not null)await c.ExecuteAsync(new CommandDefinition("INSERT INTO odca.audit_events(scope_type,tenant_id,actor_user_id,action,entity_type,entity_id,result) SELECT 'tenant',@tenantId,@actor,'billing.storage.requested','additional_storage_request',@id,'success' WHERE NOT EXISTS(SELECT 1 FROM odca.audit_events WHERE tenant_id=@tenantId AND action='billing.storage.requested' AND entity_id=@id)",new{tenantId,actor=actorId,id=row.Id},tx,cancellationToken:cancellationToken));await tx.CommitAsync(cancellationToken);return row is null?null:ToContract(row); }
    ...
    public Task<bool> ChangeOrganizationStatusAsync(Guid actorId,Guid tenantId,bool restore,string reason,CancellationToken cancellationToken)=>MutatePlatform(actorId,tenantId,async(c,tx,ct)=>
    {
        var current=await c.ExecuteScalarAsync<string?>(new CommandDefinition("SELECT status FROM odca.tenants WHERE id=@tenantId AND NOT is_deleted FOR UPDATE",new{tenantId},tx,cancellationToken:ct));
        var target=restore?"active":"suspended";
        if(current==target)return true;
        if(current!=(restore?"suspended":"active"))return false;
        var changed=await c.ExecuteAsync(new CommandDefinition("UPDATE odca.tenants SET status=@target,updated_at=now() WHERE id=@tenantId AND status=@current",new{tenantId,target,current},tx,cancellationToken:ct));
        if(changed!=1)return false;
        await c.ExecuteAsync(new CommandDefinition("INSERT INTO odca.audit_events(scope_type,tenant_id,actor_user_id,action,entity_type,entity_id,result,metadata) VALUES('tenant',@tenantId,@actor,@action,'tenant',@tenantId,'success',jsonb_build_object('reason',@reason,'previous_status',@current,'new_status',@target))",new{tenantId,actor=actorId,action=restore?"organization.restored":"organization.suspended",reason,current,target},tx,cancellationToken:ct));
        return true;
    },cancellationToken);
    ...
    private const string HistorySql="""SELECT h."Id",h."Type",h."Resource",h."Quantity",h."Unit",h."Reason",h."ActorName",h."OccurredAt" FROM (SELECT m.id AS "Id",m.movement_type AS "Type",m.resource_type AS "Resource",m.quantity AS "Quantity",m.unit AS "Unit",m.reason AS "Reason",COALESCE(u.display_name,'Processo do sistema') AS "ActorName",m.occurred_at AS "OccurredAt" FROM odca.resource_movements m LEFT JOIN odca.users u ON u.id=m.actor_user_id WHERE m.tenant_id=@tenantId UNION ALL SELECT -a.id AS "Id",a.action AS "Type",'organization' AS "Resource",0::bigint AS "Quantity",COALESCE(a.metadata->>'new_status','status') AS "Unit",a.metadata->>'reason' AS "Reason",COALESCE(u.display_name,'Processo do sistema') AS "ActorName",a.occurred_at AS "OccurredAt" FROM odca.audit_events a LEFT JOIN odca.users u ON u.id=a.actor_user_id WHERE a.tenant_id=@tenantId AND a.action IN('organization.suspended','organization.restored')) h ORDER BY h."OccurredAt" DESC LIMIT 50""";
    private const string RequestSql="""SELECT r.id AS "Id",r.package_name AS "PackageName",r.package_version::int AS "PackageVersion",r.quantity::int AS "Quantity",(r.bytes_per_unit*r.quantity)::bigint AS "TotalBytes",(r.unit_price*r.quantity) AS "TotalPrice",r.currency AS "Currency",r.terms_snapshot AS "Terms",r.status AS "Status",requester.display_name AS "RequestedBy",r.requested_at AS "RequestedAt",decider.display_name AS "DecidedBy",r.decided_at AS "DecidedAt",r.decision_reason AS "DecisionReason" FROM odca.additional_storage_requests r JOIN odca.users requester ON requester.id=r.requested_by LEFT JOIN odca.users decider ON decider.id=r.decided_by""";
    private sealed class SummaryRow { public Guid TenantId{get;set;} public string OrganizationName{get;set;}="";public string TenantStatus{get;set;}="";public string SubscriptionStatus{get;set;}="";public string PlanCode{get;set;}="";public string PlanName{get;set;}="";public int PlanVersion{get;set;}public DateTime? PeriodStart{get;set;}public DateTime? PeriodEnd{get;set;}public int ContractedSeats{get;set;}public int ActiveUsers{get;set;}public int ReservedInvitations{get;set;}public long ContractedStorageBytes{get;set;}public long AdditionalStorageBytes{get;set;}public long UsedStorageBytes{get;set;}public long ReservedStorageBytes{get;set;}public long MaximumFileBytes{get;set;} }
