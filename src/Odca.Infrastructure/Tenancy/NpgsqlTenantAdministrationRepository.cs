...
public sealed class NpgsqlTenantAdministrationRepository(NpgsqlDataSource dataSource, IDataProtectionProvider protectionProvider) : ITenantAdministrationRepository
{
    ...
    public async Task<TenantRole?> CreateRoleAsync(Guid actorId, Guid tenantId, string name, IReadOnlyCollection<string> permissions, CancellationToken cancellationToken)
    {
        ...
        if (transaction is null) return null;
        var allowed = (await connection.QueryAsync<string>(new CommandDefinition("""SELECT p.code FROM odca.permissions p WHERE p.delegable AND p.code=ANY(@permissions) AND EXISTS(SELECT 1 FROM odca.member_roles mr JOIN odca.roles ar ON ar.id=mr.role_id LEFT JOIN odca.role_permissions arp ON arp.role_id=ar.id WHERE mr.tenant_id=@tenantId AND mr.user_id=@actorId AND (ar.code='tenant-administrator' OR arp.permission_code=p.code));""",new { permissions=permissions.ToArray(),tenantId,actorId },transaction,cancellationToken:cancellationToken))).ToArray();
    ...
    public async Task<InvitationRecord?> CreateInvitationAsync(Guid actorId, Guid tenantId, string email, Guid roleId, string idempotencyKey, CancellationToken cancellationToken)
    {
        await using var connection=await dataSource.OpenConnectionAsync(cancellationToken);
