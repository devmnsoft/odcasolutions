...
public sealed class ObligationsController(NpgsqlDataSource dataSource) : ControllerBase
{
    ...
    public async Task<IActionResult> Create(Guid tenantId,Guid contractId,[FromBody] CreateObligationRequest request,CancellationToken ct)
    {
        ...
        if(request.Recurrence is { IntendedDay: <1 or >31 } || request.Recurrence is { OccurrenceCount: >120 } || request.Recurrence is { EndsOn: null, OccurrenceCount: null }) return ValidationProblem();
        ...
        List<DateOnly> dates=request.Recurrence is null?[request.DueDate]:Materialize(request.Recurrence);
    ...
    public async Task<IActionResult> Action(Guid tenantId,Guid id,string operation,[FromBody] ObligationActionRequest request,CancellationToken ct)
    {
        var actor=Actor(); if(actor is null)return Unauthorized(); var permission=operation switch{"fulfill"=>"tenant.obligations.fulfill","reopen"=>"tenant.obligations.reopen","cancel"=>"tenant.obligations.cancel","reassign"=>"tenant.obligations.assign",_=>"tenant.obligations.manage"};
        await using var c=await dataSource.OpenConnectionAsync(ct);if(!await Allowed(c,actor.Value,tenantId,permission,ct))return Forbid();if(request.EvidenceDocumentVersionId.HasValue&&!await Allowed(c,actor.Value,tenantId,"tenant.documents.download",ct))return Forbid();await using var tx=await c.BeginTransactionAsync(ct);await SetTenant(c,tenantId,tx,ct);
        ...
        switch(operation){case "start" when status=="open":status="in_progress";break;case "fulfill" when status is "open" or "in_progress":if(item.EvidenceRequired&&(!request.EvidenceDocumentVersionId.HasValue||request.EvidenceDocumentVersionId.Value==Guid.Empty))return ValidationProblem();if(request.EffectiveAt.HasValue){fulfilled=request.EffectiveAt.Value;}else{var tenantInfo=await c.QuerySingleAsync<(string Timezone,DateOnly Today)>(new CommandDefinition("SELECT timezone AS Timezone,(now() AT TIME ZONE timezone)::date AS Today FROM odca.tenants WHERE id=@tenantId",new{tenantId},tx,cancellationToken:ct));var tz=TimeZonePolicy.Resolve(tenantInfo.Timezone,"America/Sao_Paulo");var localDateTime=tenantInfo.Today.ToDateTime(TimeOnly.MinValue);var offset=tz.GetUtcOffset(localDateTime);fulfilled=new DateTimeOffset(localDateTime,offset);}break;case "cancel" when status is "open" or "in_progress":if(string.IsNullOrWhiteSpace(request.Reason))return ValidationProblem();status="cancelled";break;case "reopen" when status is "fulfilled" or "cancelled":if(string.IsNullOrWhiteSpace(request.Reason))return ValidationProblem();status="open";break;case "reschedule" when status is "open" or "in_progress":if(request.DueDate is null||request.DueDate==item.DueDate||string.IsNullOrWhiteSpace(request.Reason))return ValidationProblem();due=request.DueDate.Value;break;case "reassign" when status is "open" or "in_progress":if(!request.OwnerId.HasValue||request.OwnerId.Value==Guid.Empty||request.OwnerId.Value==item.OwnerId||string.IsNullOrWhiteSpace(request.Reason))return ValidationProblem();var active=await c.ExecuteScalarAsync<bool>(new CommandDefinition("SELECT EXISTS(SELECT 1 FROM odca.memberships WHERE tenant_id=@tenantId AND user_id=@owner AND status='active')",new{tenantId,owner=request.OwnerId},tx,cancellationToken:ct));if(!active)return ValidationProblem();owner=request.OwnerId.Value;break;default:return Conflict(new{title="Transição de estado não permitida."});}
        if(operation=="fulfill")status="fulfilled";
        await c.ExecuteAsync(new CommandDefinition("""
          UPDATE odca.contract_obligations SET status=@status,due_date=@due,owner_id=@owner,row_version=row_version+1,
            fulfilled_at=CASE WHEN @operation='fulfill' THEN @fulfilled WHEN @operation='reopen' THEN NULL ELSE fulfilled_at END,
            fulfilled_by=CASE WHEN @operation='fulfill' THEN @actor WHEN @operation='reopen' THEN NULL ELSE fulfilled_by END,
            fulfillment_note=CASE WHEN @operation='fulfill' THEN @note WHEN @operation='reopen' THEN NULL ELSE fulfillment_note END,
          ...
          VALUES(@tenantId,@id,@actor,@operation,jsonb_strip_nulls(jsonb_build_object(
          ...
          """,new{status,due,owner,fulfilled,actor,note=request.Note,evidence=request.EvidenceDocumentVersionId,tenantId,id,operation,reason=request.Reason,previousDue=item.DueDate,previousOwner=item.OwnerId},tx,cancellationToken:ct));
        if(request.EvidenceDocumentVersionId is Guid evidence){var valid=await c.ExecuteScalarAsync<bool>(new CommandDefinition("SELECT EXISTS(SELECT 1 FROM odca.document_versions v JOIN odca.contract_documents d ON d.id=v.document_id AND d.tenant_id=v.tenant_id JOIN odca.contract_obligations o ON o.contract_id=v.contract_id AND o.tenant_id=v.tenant_id WHERE o.id=@id AND v.id=@evidence AND v.tenant_id=@tenantId AND v.security_status='safe' AND d.deleted_at IS NULL)",new{tenantId,id,evidence},tx,cancellationToken:ct));if(!valid)return ValidationProblem(new ValidationProblemDetails(new Dictionary<string,string[]>{{"evidenceDocumentVersionId",["A evidência precisa ser uma versão segura e ativa de um documento deste contrato."]}}));await c.ExecuteAsync(new CommandDefinition("INSERT INTO odca.obligation_evidence(tenant_id,obligation_id,document_version_id,linked_by) VALUES(@tenantId,@id,@evidence,@actor) ON CONFLICT DO NOTHING",new{tenantId,id,evidence,actor},tx,cancellationToken:ct));}
        if(operation=="reschedule")foreach(var days in new[]{7,1,0})await c.ExecuteAsync(new CommandDefinition("INSERT INTO odca.obligation_reminders(tenant_id,obligation_id,days_before,deduplication_key,scheduled_for) VALUES(@tenantId,@id,@days,@key,@scheduled) ON CONFLICT DO NOTHING",new{tenantId,id,days,key=$"{id:N}:v{item.Version+1}:d{days}",scheduled=due.AddDays(-days)},tx,cancellationToken:ct));
    ...
    private static Task<bool> Allowed(NpgsqlConnection c,Guid actor,Guid tenant,string permission,CancellationToken ct,NpgsqlTransaction? tx=null)=>c.ExecuteScalarAsync<bool>(new CommandDefinition("SELECT odca.tenant_actor_has_permission(@actor,@tenant,@permission)",new{actor,tenant,permission},tx,cancellationToken:ct));
