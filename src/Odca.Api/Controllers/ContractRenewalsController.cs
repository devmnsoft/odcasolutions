...
public sealed class ContractRenewalsController(NpgsqlDataSource dataSource) : ControllerBase
{
    ...
    {
        var actor=Actor();if(actor is null)return Unauthorized();var allowed=new[]{"intent_to_renew","negotiating","not_renewing","renewed"};if(!allowed.Contains(request.Decision)||string.IsNullOrWhiteSpace(request.Justification))return ValidationProblem();
        ...
        if(request.Decision=="renewed"&&(request.NewStartDate is null||request.NewEndDate is null||request.NewEndDate<request.NewStartDate))return ValidationProblem(new ValidationProblemDetails(new Dictionary<string,string[]>{{"newEndDate",["Registrar renovação exige novas datas válidas."]}}));
        ...
        if(request.Decision=="renewed")await c.ExecuteAsync(new CommandDefinition("UPDATE odca.contracts SET start_date=@start,end_date=@end,version=version+1,updated_at=now() WHERE tenant_id=@tenantId AND id=@contractId",new{start=request.NewStartDate,end=request.NewEndDate,tenantId,contractId},tx,cancellationToken:ct));
        ...
        return Ok(new{id,cycle,decision=request.Decision,contractVersion=request.Decision=="renewed"?contract.Version+1:contract.Version,obligationsPreserved=true});
    }
