...
-- ODCA-MIGRATION 009 CHECKSUM 8ea94f7cafca871d9a1923a8383eaf89a7535f2749efa550e6930bfcf7a7982b
BEGIN;
SELECT pg_advisory_xact_lock(hashtext('odca.schema.migrations'));
...
CREATE OR REPLACE FUNCTION odca.preview_tenant_invitation(invitation_id uuid, presented_hash text)
RETURNS TABLE(
  invitation_id uuid,
  organization_name text,
  role_name text,
...
DECLARE
  locked_tenant_id uuid;
BEGIN
  SELECT i.tenant_id INTO locked_tenant_id FROM odca.tenant_invitations i WHERE i.id = invitation_id;
  IF FOUND THEN
    PERFORM odca.expire_tenant_invitations(locked_tenant_id);
  END IF;
...
INSERT INTO odca.schema_migrations(version,name,checksum)
VALUES (9,'S01 invitation lifecycle and last-admin guards','8ea94f7cafca871d9a1923a8383eaf89a7535f2749efa550e6930bfcf7a7982b')
ON CONFLICT (version) DO NOTHING;
COMMIT;
-- ODCA-END 009
-- ODCA Solutions migration 010: S02 contracts center (counterparties, types, contracts, alerts).
-- ODCA-MIGRATION 010 CHECKSUM 9abdb1e69f064536de7915658f42553fbb7f481fc0f62d41ee1019a32d8c1869
BEGIN;
SELECT pg_advisory_xact_lock(hashtext('odca.schema.migrations'));

INSERT INTO odca.permissions (code, description, delegable) VALUES
 ('tenant.counterparties.read', 'Consultar contrapartes da organização.', true),
 ('tenant.counterparties.manage', 'Cadastrar e gerenciar contrapartes.', true),
 ('tenant.contract_types.read', 'Consultar tipos de contrato.', true),
 ('tenant.contract_types.manage', 'Cadastrar e gerenciar tipos de contrato.', true),
 ('tenant.contracts.read', 'Consultar contratos da organização.', true),
 ('tenant.contracts.manage', 'Criar e editar contratos em rascunho ou ativos.', true),
 ('tenant.contracts.lifecycle', 'Ativar, renovar, encerrar ou cancelar contratos.', true),
 ('tenant.notifications.read', 'Consultar notificações internas do usuário.', true)
ON CONFLICT (code) DO UPDATE SET description = EXCLUDED.description, delegable = EXCLUDED.delegable;

INSERT INTO odca.role_permissions(role_id, permission_code)
SELECT r.id, p.code
  FROM odca.roles r
  CROSS JOIN odca.permissions p
 WHERE r.scope_type = 'tenant'
   AND r.code = 'tenant-administrator'
   AND p.code LIKE 'tenant.%'
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS odca.counterparties
(
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id uuid NOT NULL REFERENCES odca.tenants(id),
    person_type text NOT NULL CHECK (person_type IN ('individual', 'organization')),
    legal_name text NOT NULL,
    display_name text NOT NULL,
    document_type text NOT NULL CHECK (document_type IN ('cpf', 'cnpj', 'none')),
    ...
    inactivated_by uuid REFERENCES odca.users(id),
    version bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    is_deleted boolean NOT NULL DEFAULT false,
    deleted_at timestamptz,
    deleted_by uuid REFERENCES odca.users(id),
    deletion_reason text,
    CONSTRAINT counterparties_document_presence_ck CHECK
    (
        (document_type = 'none' AND document_normalized IS NULL AND document_display IS NULL)
        OR (document_type <> 'none' AND document_normalized IS NOT NULL AND length(document_normalized) > 0)
    ),
    CONSTRAINT counterparties_role_ck CHECK (is_client OR is_supplier OR is_partner OR is_provider),
    CONSTRAINT counterparties_deletion_state_ck CHECK
    (
        (NOT is_deleted AND deleted_at IS NULL AND deleted_by IS NULL AND deletion_reason IS NULL)
        OR (is_deleted AND deleted_at IS NOT NULL AND deleted_by IS NOT NULL AND deletion_reason IS NOT NULL)
    ),
    CONSTRAINT counterparties_inactive_state_ck CHECK
    (
...
CREATE UNIQUE INDEX IF NOT EXISTS counterparties_live_document_uq
    ON odca.counterparties (tenant_id, document_type, document_normalized)
    WHERE NOT is_deleted
      AND document_type <> 'none'
      AND document_normalized IS NOT NULL;

CREATE INDEX IF NOT EXISTS counterparties_tenant_name_ix
    ON odca.counterparties (tenant_id, lower(display_name))
    WHERE NOT is_deleted;

CREATE TABLE IF NOT EXISTS odca.contract_types
(
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id uuid NOT NULL REFERENCES odca.tenants(id),
    code text NOT NULL,
    ...
    guidance text,
    status text NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'inactive')),
    is_system_demo boolean NOT NULL DEFAULT false,
    created_at timestamptz NOT NULL DEFAULT now(),
    created_by uuid REFERENCES odca.users(id),
    updated_at timestamptz NOT NULL DEFAULT now(),
    version bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    UNIQUE (tenant_id, id),
...
CREATE TABLE IF NOT EXISTS odca.contracts
(
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id uuid NOT NULL REFERENCES odca.tenants(id),
    ...
    primary_counterparty_id uuid NOT NULL,
    owner_user_id uuid REFERENCES odca.users(id),
    start_date date NOT NULL,
    end_date date,
    is_indefinite boolean NOT NULL DEFAULT false,
    amount numeric(18,2),
    currency char(3),
    amount_periodicity text CHECK (amount_periodicity IS NULL OR amount_periodicity IN ('once', 'monthly', 'yearly', 'other')),
    ...
    renewal_decision text NOT NULL DEFAULT 'pending'
        CHECK (renewal_decision IN ('pending', 'renew', 'do_not_renew', 'not_applicable')),
    operational_status text NOT NULL DEFAULT 'draft'
        CHECK (operational_status IN ('draft', 'active', 'closed', 'cancelled', 'inactive')),
    version bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    term_cycle integer NOT NULL DEFAULT 1 CHECK (term_cycle > 0),
    created_at timestamptz NOT NULL DEFAULT now(),
    created_by uuid NOT NULL REFERENCES odca.users(id),
    updated_at timestamptz NOT NULL DEFAULT now(),
    closed_at timestamptz,
    closed_by uuid REFERENCES odca.users(id),
    cancelled_at timestamptz,
    cancelled_by uuid REFERENCES odca.users(id),
    is_deleted boolean NOT NULL DEFAULT false,
    deleted_at timestamptz,
    deleted_by uuid REFERENCES odca.users(id),
    deletion_reason text,
    CONSTRAINT contracts_term_ck CHECK
    (
        (is_indefinite AND end_date IS NULL)
        OR (NOT is_indefinite AND end_date IS NOT NULL AND end_date >= start_date)
    ),
    CONSTRAINT contracts_amount_currency_ck CHECK
    (
        (amount IS NULL AND currency IS NULL)
        OR (amount IS NOT NULL AND currency IS NOT NULL AND length(currency) = 3)
    ),
    CONSTRAINT contracts_deletion_state_ck CHECK
    (
        (NOT is_deleted AND deleted_at IS NULL AND deleted_by IS NULL AND deletion_reason IS NULL)
        OR (is_deleted AND deleted_at IS NOT NULL AND deleted_by IS NOT NULL AND deletion_reason IS NOT NULL)
    ),
    UNIQUE (tenant_id, id),
    FOREIGN KEY (tenant_id, type_id) REFERENCES odca.contract_types (tenant_id, id),
    FOREIGN KEY (tenant_id, primary_counterparty_id) REFERENCES odca.counterparties (tenant_id, id)
);

CREATE INDEX IF NOT EXISTS contracts_tenant_status_ix
    ON odca.contracts (tenant_id, operational_status)
    WHERE NOT is_deleted;
CREATE INDEX IF NOT EXISTS contracts_tenant_end_ix
    ON odca.contracts (tenant_id, end_date)
    WHERE NOT is_deleted AND end_date IS NOT NULL;
CREATE INDEX IF NOT EXISTS contracts_tenant_owner_ix
    ON odca.contracts (tenant_id, owner_user_id)
    WHERE NOT is_deleted;

CREATE TABLE IF NOT EXISTS odca.contract_parties
(
    contract_id uuid NOT NULL,
    tenant_id uuid NOT NULL,
    counterparty_id uuid NOT NULL,
    role text NOT NULL CHECK (role IN ('primary', 'additional', 'guarantor', 'other')),
    created_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (contract_id, counterparty_id),
    UNIQUE (tenant_id, contract_id, counterparty_id),
    FOREIGN KEY (tenant_id, contract_id) REFERENCES odca.contracts (tenant_id, id) ON DELETE CASCADE,
    FOREIGN KEY (tenant_id, counterparty_id) REFERENCES odca.counterparties (tenant_id, id)
);

CREATE TABLE IF NOT EXISTS odca.contract_renewals
(
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id uuid NOT NULL,
    contract_id uuid NOT NULL,
    ...
    previous_term_cycle integer NOT NULL CHECK (previous_term_cycle > 0),
    new_term_cycle integer NOT NULL CHECK (new_term_cycle > previous_term_cycle),
    reason text,
    created_by uuid NOT NULL REFERENCES odca.users(id),
    created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS contract_renewals_contract_ix
    ON odca.contract_renewals (tenant_id, contract_id, created_at DESC);

CREATE TABLE IF NOT EXISTS odca.contract_events
(
    id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    tenant_id uuid NOT NULL,
    contract_id uuid NOT NULL,
...
CREATE INDEX IF NOT EXISTS contract_events_contract_ix
    ON odca.contract_events (tenant_id, contract_id, occurred_at DESC);

CREATE TABLE IF NOT EXISTS odca.user_notifications
(
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id uuid NOT NULL REFERENCES odca.tenants(id),
    user_id uuid NOT NULL REFERENCES odca.users(id),
    category text NOT NULL CHECK (category IN ('contract_expiry', 'renewal_notice', 'contract_attention')),
    title text NOT NULL,
    body text NOT NULL,
    resource_type text NOT NULL,
    resource_id uuid NOT NULL,
    event_key text NOT NULL,
    status text NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'obsolete')),
...
CREATE INDEX IF NOT EXISTS user_notifications_user_open_ix
    ON odca.user_notifications (tenant_id, user_id, created_at DESC)
    WHERE status = 'open' AND NOT is_obsolete;

ALTER TABLE odca.counterparties ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.counterparties FORCE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_types ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_types FORCE ROW LEVEL SECURITY;
ALTER TABLE odca.contracts ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.contracts FORCE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_parties ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_parties FORCE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_renewals ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_renewals FORCE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.contract_events FORCE ROW LEVEL SECURITY;
ALTER TABLE odca.user_notifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE odca.user_notifications FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tenant_isolation ON odca.counterparties;
CREATE POLICY tenant_isolation ON odca.counterparties TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON odca.contract_types;
CREATE POLICY tenant_isolation ON odca.contract_types TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON odca.contracts;
CREATE POLICY tenant_isolation ON odca.contracts TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON odca.contract_parties;
CREATE POLICY tenant_isolation ON odca.contract_parties TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON odca.contract_renewals;
CREATE POLICY tenant_isolation ON odca.contract_renewals TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON odca.contract_events;
CREATE POLICY tenant_isolation ON odca.contract_events TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation ON odca.user_notifications;
CREATE POLICY tenant_isolation ON odca.user_notifications TO odca_app
    USING (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('odca.tenant_id', true), '')::uuid);

GRANT SELECT, INSERT, UPDATE ON odca.counterparties, odca.contract_types, odca.contracts,
    odca.contract_parties, odca.contract_renewals, odca.user_notifications TO odca_app;
GRANT SELECT, INSERT ON odca.contract_events TO odca_app;
GRANT USAGE, SELECT ON SEQUENCE odca.contract_events_id_seq TO odca_app;

CREATE OR REPLACE FUNCTION odca.list_contract_alert_tenants()
...
AS 
  SELECT t.id, t.timezone
   ...
   WHERE NOT t.is_deleted
     AND t.status = 'active';
;
REVOKE ALL ON FUNCTION odca.list_contract_alert_tenants() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION odca.list_contract_alert_tenants() TO odca_app;

INSERT INTO odca.schema_migrations (version, name, checksum)
VALUES (10, 'S02 contracts center', '9abdb1e69f064536de7915658f42553fbb7f481fc0f62d41ee1019a32d8c1869')
ON CONFLICT (version) DO NOTHING;
